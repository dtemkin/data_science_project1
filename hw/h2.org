...
- Add a README.md file with headers describing  *Project objective* and *Milestones per week* upto 8th March.
  .Rmd and .md has same markdown format. 
  
  We will revised milestones as per progress, client update and course requirements. 
...
- Most of the projects have excel file sheets.
   -  Write a function for data importing in a separate file called import_tidy.r. 
     ...
     Look at  *http://readxl.tidyverse.org/* for more information. *For csv file read_csv is a good option.*

     - Write an function to get data in tidy format. Write some initial code and as we move ahead will improve it.

- If you are downloading data from the web(rss feed or images).
  - Write functions for importing the data and setting up the pipeline for storing and loading it.
  - Write functions for cleaning and transforming data. 
     - For text data it can related to normalization operations on the text data.
     - For image data it can be taking care of outlier pixels and normalizing(using $l_2 norm, \infinity(max) norm$ etc.) the data.

*If you already ahead in the project, let me know which portion (file name and line numbers) of your code I should look into.*
*If I am not added to your project, then you need to show the code in your computer.*
